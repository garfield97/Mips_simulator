...
void r_type(memory &memory, registers &registers, program_counter &PC, const unsigned int instruction)
{
    ...
    switch (opcode)
    {
        ...
        case 0x09: registers.reg[rt] = abs(registers.reg[rs]) + abs(IMM);
        case 0x0A: if (registers.reg[rs] < IMM) registers.reg[rt] = 1;
        case 0x0B: if (registers.reg[rs] < abs(IMM)) registers.reg[rt] = 1;
        ...
        case 0x24: registers.reg[rt] = abs(memory.load_byte(registers.reg[rs]));
        case 0x25: registers.reg[rt] = abs(memory.load_hword(registers.reg[rs]));
